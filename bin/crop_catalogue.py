...
coords = SkyCoord(data["RAJ2000"], data["DEJ2000"], unit=(u.deg, u.deg))
...
alphacut = np.where(np.logical_not(np.isnan(data["alpha"])))
...
if options.attenuate:
    if options.metafits:
        x,y = beam_value(data["RAJ2000"][indices],data["DEJ2000"][indices],options.metafits)
